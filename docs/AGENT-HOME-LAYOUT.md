...
For shipped roles (`eng`, `lead`, `critic`, `researcher`, `explorer`),
this is sourced from `templates/agents/<role>/AGENTS.md` and copied
into the agent's home by `metasphere agent seed`.
...
For non-orchestrator agents (`eng`, `lead`, `critic`, `researcher`,
`explorer`), use the seeder:
...
| Use case | Long-running coordinator, monitor, team member | Single well-scoped task |
