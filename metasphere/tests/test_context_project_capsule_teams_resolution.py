...
def test_resolution_frontmatter_overrides_teams_yaml(tmp_paths: Paths):
    ...
    _seed_project_file(tmp_paths, "worldwire", learnings="teams-loses")
    _write_teams_yaml(tmp_paths, """
agents:
  override-probe:
    projects: [worldwire]
...
def test_resolution_teams_yaml_overrides_path_inference(tmp_paths: Paths):
    ...
    _seed_project_nested_agent_mission(
        tmp_paths, "worldwire", "@nested-probe",
    )
    _seed_project_file(
        tmp_paths, "worldwire", learnings="path-inference-result",
    )
    _seed_project_file(
...
def test_resolution_path_nested_fallback_when_teams_empty(tmp_paths: Paths):
    ...
    _seed_project_nested_agent_mission(
        tmp_paths, "worldwire", "@b4-fallback",
    )
    _seed_project_file(
        tmp_paths, "worldwire", learnings="path-fallback-result",
    )
...
def test_resolution_b7_name_prefix_inference_REMOVED(tmp_paths: Paths):
    """Critical regression: ``@worldwire-eng-fresh`` at root scope
    with project ``worldwire`` registered and NO teams.yaml entry +
    NO frontmatter must NOT auto-resolve. Name-prefix string-match
    is gone."""
    _seed_root_agent_mission(tmp_paths, "@worldwire-eng-fresh")
    _seed_project_file(
        tmp_paths, "worldwire", learnings="must-not-render",
    )
...
agents:
  somebody-else:
    projects: [writing]
""")
    out = ctx._render_project_capsule(tmp_paths, "@worldwire-eng-fresh")
    assert out == ""
...
def test_resolution_no_teams_yaml_file_falls_through(tmp_paths: Paths):
    ...
    _seed_project_nested_agent_mission(
        tmp_paths, "worldwire", "@nested-no-teams",
    )
    _seed_project_file(
        tmp_paths, "worldwire", learnings="path-only-result",
    )
