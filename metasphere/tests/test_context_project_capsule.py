...
def test_scalar_project_field_loads_files(tmp_paths: Paths):
    ...
    _seed_project_files(
        ...
        memory="memory: ricardo owns Louvain inheritance.",
    )
...
def test_list_projects_field_concats_in_order(tmp_paths: Paths):
    _seed_agent_mission(
        tmp_paths, "@beta",
        "projects: [recurse, recurse-server]\n",
    )
    _seed_project_files(
        tmp_paths, "recurse", learnings="sense-making substrate.",
    )
    _seed_project_files(
        tmp_paths, "recurse-server", memory="ops contact: alex.",
    )

    out = ctx._render_project_capsule(tmp_paths, "@beta")

    idx_recurse = out.find("## Project: recurse\n")
    idx_server = out.find("## Project: recurse-server")
    assert idx_recurse >= 0
    assert idx_server > idx_recurse
    assert "sense-making substrate." in out
    assert "ops contact: alex." in out
