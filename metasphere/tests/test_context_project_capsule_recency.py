...
def test_capsule_inbox_independence(tmp_paths: Paths):
    ...
    _seed_project_nested_agent_mission(
        tmp_paths, "worldwire", "@indep-probe",
    )
    content = "\n\n".join(
        f"## 2026-{(i % 12) + 1:02d}-15: entry {i}\n\nbody {i}"
        for i in range(8)
    )
    _seed_project_file(tmp_paths, "worldwire", "LEARNINGS.md", content)
...
def test_capsule_t1_explicit_frontmatter_regression(tmp_paths: Paths):
    ...
    _seed_root_agent_mission(
        tmp_paths, "@t1-probe", frontmatter="project: worldwire\n",
    )
    _seed_project_file(
        tmp_paths, "worldwire", "LEARNINGS.md",
        "## 2026-05-29: entry\n\nbody\n",
...
def test_capsule_recency_newest_first_e2e(tmp_paths: Paths):
    ...
    _seed_project_nested_agent_mission(
        tmp_paths, "worldwire", "@recency-probe",
    )
    content = (
    ...
    _seed_project_file(
        tmp_paths, "worldwire", "LEARNINGS.md", content,
    )
