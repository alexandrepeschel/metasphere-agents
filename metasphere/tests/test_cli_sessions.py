...
def test_list_with_agents(monkeypatch, capsys):
    monkeypatch.setattr(
        ...
        lambda: [
            ...
            (_FakeAgent(name="@worldwire-eng", project="worldwire"),
             "metasphere-worldwire-worldwire-eng"),
        ],
    )
    rc, out, _ = _run(["list"], capsys)
    assert rc == 0
    ...
    assert "@worldwire-eng [worldwire]" in out
    assert "metasphere-worldwire-worldwire-eng" in out
