...
def test_matches_in_learnings_emits_nudge(tmp_paths: Paths):
    _register_project(tmp_paths, "worldwire")
    _register_project(tmp_paths, "recurse")
    _seed_agent(
        tmp_paths, "@alpha",
        learnings=(
            "- worldwire VACUUM lessons at 20M chunks.\n"
            "- generic coordination habit unrelated.\n"
            "- recurse sense-making substrate.\n"
        ),
    )
    out = ctx._render_project_migration_nudge(tmp_paths, "@alpha")
...
def test_case_insensitive_match(tmp_paths: Paths):
    ...
    _seed_agent(
        tmp_paths, "@alpha",
        learnings="- WorldWire pipeline architecture.\n",
    )
    out = ctx._render_project_migration_nudge(tmp_paths, "@alpha")
    assert "## Per-project memory migration" in out
    assert "worldwire" in out
...
def test_match_in_memory_file_also_counts(tmp_paths: Paths):
    ...
    _seed_agent(
        tmp_paths, "@alpha",
        memory="- worldwire monitor reads public.articles directly.\n",
    )
    out = ctx._render_project_migration_nudge(tmp_paths, "@alpha")
    assert "worldwire" in out
...
def test_repeat_invocation_unchanged_files_suppressed(tmp_paths: Paths):
    ...
    _seed_agent(
        tmp_paths, "@alpha",
        learnings="- worldwire VACUUM lessons.\n",
    )
...
def test_build_context_emits_nudge_section(tmp_paths: Paths):
    ...
    _seed_agent(
        tmp_paths, "@e2e",
        learnings="- worldwire monitoring topology.\n",
    )
