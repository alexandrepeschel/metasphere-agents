...
""")
    ...
    assert out == {
        "worldwire-eng": ["worldwire"],
        "orchestrator": ["worldwire", "metasphere-agents", "writing"],
    }
...
""")
    out = _teams._load_teams_config(tmp_paths)
    assert out == {"worldwire-eng": ["worldwire", "cam"]}
...
""")
    ...
    assert first == {"worldwire-eng": ["worldwire"]}
    ...
    assert cached == {"worldwire-eng": ["worldwire"]}
...
agents:
  worldwire-eng:
    projects: [worldwire]
""")
    with_at = _teams._lookup_agent_projects("@worldwire-eng", tmp_paths)
    without_at = _teams._lookup_agent_projects("worldwire-eng", tmp_paths)
    assert with_at == ["worldwire"]
    assert without_at == ["worldwire"]
...
agents:
  orchestrator:
    projects: [worldwire, metasphere-agents, writing, cam]
""")
    out = _teams._lookup_agent_projects("@orchestrator", tmp_paths)
    assert out == ["worldwire", "metasphere-agents", "writing", "cam"]
