...
- **B4 auto-inference — frontmatter becomes explicit-override, not required.** Trigger: `@worldwire-eng`'s MISSION.md predates the per-project memory layer and has no `project:` key, so the T1 capsule no-op'd and the agent couldn't see worldwire's LEARNINGS. Per Julian (2026-05-29 22:32Z, msg-1780093638): "Why do we need the frontmatter? We shouldn't rely on that." Resolution chain extended to: (1) MISSION.md frontmatter override; (2) path inference (`~/.metasphere/projects/<P>/agents/@<agent>/` → `<P>`); (3) name-prefix inference (`@<project>-<role>` + `projects/<project>/` exists). Multi-project agents still go through frontmatter. (`b245aa3` #124)
- **B5 relevance-rank entries inside the budget (later replaced).** Canonical Hetzner-tunnel-670s end-to-end on @worldwire-eng still failed after B4: the real LEARNINGS.md is ~38KB and the head-of-file byte-truncator buried the load-bearing entry past the 2KB cap. Replaced `_read_capped`'s slice with entry-aware relevance: parse atomic `(header, body)` entries at ##/### boundaries, tokenize last 5 inbox bodies + MISSION.md body into a stopword-filtered query set, score by `|query ∩ entry_tokens|`, greedy-fill within budget in score-desc order. (`c21e1a7` #125)
- **B6 replace relevance-rank with recency-sort + always-include file-path pointer.** Julian's 2026-05-29 23:48Z directive (orch msg-1780098877): query-based ranking was over-engineered, wake-banner timing made the inbox signal unreliable, recency is predictable. Agents who need older content grep the file directly via the new footer pointer. Deleted `_extract_query_signal`, `_tokenize_query`, `_rank_entries`, the stopwords, the inbox-touching code path, and `_fit_head_entries`. New `_sort_entries_by_recency` parses ISO date from each header, sorts dated entries newest-first, undated entries land after in original document order. Footer pointer to the on-disk file is always present so the agent has a path even when budget displaces everything. (`eac2bfc` #126)
- **B7 central `teams.yaml` config — drop name-prefix string-match inference entirely.** Julian's same 2026-05-29 23:48Z directive flagged the brittleness: the B4 name-prefix branch first-dash-splits `@<a-b>-<c>` to `<a-b>`, which makes `@polymarket-agents-research` resolve to "polymarket" (no such project) rather than "polymarket-agents". Quote: "matching the agent identity by string matching is brittle. We could have a central config file where agent teams are registered." New `metasphere/teams.py` loader + lookup against `~/.metasphere/teams.yaml` (schema: `agents.<name>.projects: [...]`). Graceful-degrades on every failure mode so the resolution chain falls through to path inference. Mtime-keyed cache, O(1) once warm; operator edits land within one tick. `templates/install/teams.yaml` ships a canonical roster with schema notes. (`5a7f29a` #127)
- **B8 runtime nudge — AGENTS.md tells agents to grep the file when asked about something outside the recency window.** Closes the behavioral gap from orch's @worldwire-eng re-test: capsule rendered correctly but the agent didn't grep when asked about an entry outside the budget — it answered "not in memory" and fell back to auto-memory. Julian 2026-05-30 00:18Z (orch msg-1780100374): runtime nudge in AGENTS.md, not directive-grep — "I don't want them to always grep that could dilute their reasoning." Adds an identical "## Project memory store" section to every role template (critic, designer, eng, explorer, lead, orchestrator, researcher) with the three-part frame: capsule = recency lens, LEARNINGS.md / MEMORY.md at the footer path = primary memory store, auto-memory = last resort. (`c56d4c6` #128)
- **Repo-level docs alignment — README + install-templates + reference docs.** Julian 2026-05-30 00:30Z (orch msg-1780101086). README "Agent memory" bullet expanded into "Per-project memory" + "Agent memory" so first-time readers see the team-shared layer first; brief Memory section replaced with a layered walkthrough (per-project = LEARNINGS lessons + MEMORY facts → agent-level → auto-memory) including sample footer pointer + three-step resolution chain. (`611141d` #129; preceded by the earlier README-only landing `da9da71`)
- **Researcher template class-level discipline.** Side fix landed mid-tranche: W21 weekly digest from `@recurse-research` was wrong because their MISSION.md baked a stale canonical path (`~/.openclaw/workspace/repos/writing/` workspace clone frozen 2026-04-04, identical dir-shape to live `/home/openclaw/projects/writing/`). Same-shape dirs silently returned "no recent files" instead of error — same failure class as W18-W19 stale `~/.openclaw/skills/research-monitor/` shadows. Adds a "Canonical source verification" section to `templates/agents/researcher/AGENTS.md` mandating `git -C PATH remote -v` + `git -C PATH log -1 --format=%cI` on every read window for git-backed sources, mtime check for non-git. (`e38be6e` #121)
...
- **Auto-escalate undelivered high-priority wakes to session-respawn.** Trigger: msg-1780086179 (`@writing-lead`'s !task sat 7h unread before Julian asked for the drafts). When `send_message`'s best-effort `wake_recipient_if_live` inject returns `delivered=False` (dormant session, deferred typing, unresolvable target) and the label is `!task` / `!urgent` / `!query`, escalate to `agents.wake_persistent`. The `delivered` bool was added for #106 but was discarded at the send_message call site, so high-priority work could sit unread for hours on a nominally-alive but unresponsive recipient. Julian greenlight at 20:24Z (msg-1780086202) on `!task` + `!urgent`; widened to include `!query` at 20:26Z (msg-1780086430) — `!query` semantically requires a response, silent unread defeats the contract. `!info` / `!done` / `!reply` intentionally stay on heartbeat cadence (async status flow where REPL pickup on the next turn is the correct semantic). Guards on self-send, project recipient, @user sender, and `wake=False` sends. (`7bc3efe` #123)

### audit-docs filter refinements

Three small filters that cut false positives Julian was stripping by hand on every CHANGELOG promotion (visible as the "Five non-bump commits" / "Six non-bump commits" framing in the last four entries).
